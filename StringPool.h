...
namespace PinInCpp {
	...
	public:
		...
		virtual std::string_view getchar_view(size_t i)const = 0;//获取指定字符的只读视图
		virtual std::string_view getstr_view(size_t strStart)const = 0;//输入首端索引构造完整字符串的只读视图
	...
	*/
	...
	public:
		...
		virtual std::string_view getchar_view(size_t i)const;//获取指定字符的只读视图 持有时不要变动字符串池！
		virtual std::string_view getstr_view(size_t strStart)const;//输入首端索引构造完整字符串的只读视图 持有时不要变动字符串池！
